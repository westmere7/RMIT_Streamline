...
describe("ItemDetailPanel", () => {
  ...
  it("lists linked items from other boards and opens the link dialog", async () => {
    ...
    expect(linked).toHaveTextContent("Syncs 12 fields");
    expect(within(linked).getByTitle(/Syncs assets, name, description, Owner, Status, Priority, Timeline, Due Date, Requester, Department, Size, Brief/)).toBeInTheDocument();
