...
test.describe("board lifecycle", () => {
  ...
  test("the Blank template builds the groups and columns it promises", async ({ page }) => {
    ...
    for (const [template, want] of Object.entries(expected)) {
      ...
      // Ticket is a fixed slot in front of the name, not one of the board's columns.
      const headers = (await page.getByRole("columnheader").allTextContents()).filter((h) => h !== "Ticket");
