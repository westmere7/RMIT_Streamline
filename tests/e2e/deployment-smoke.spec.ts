...
/**
 * Removes the run's item if it is still there. Called from `afterEach`, which has
 * its own time budget, so a run that fails or times out halfway through still
 * leaves the workspace as it found it instead of a stray "deploy-… smoke item".
 */
...
test.describe("deployed build", () => {
  /** Name of the item the lifecycle test created, for afterEach to remove if it is still there. */
  ...
  test("creates an item, works on it, and removes it again", async ({ page }) => {
    ...
    const from = (await row(page, name).getByTestId("item-drag-area").boundingBox())!;
