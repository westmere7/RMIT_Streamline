...
export async function seedDatabase(db: StreamlineDatabase, now: Date = new Date()): Promise<void> {
  ...
  await Promise.all([
    ...
    ...seed.workspaceInvitations.map((r) => tx.objectStore("workspaceInvitations").put(r)),
    ...
    ...seed.itemLinks.map((r) => tx.objectStore("itemLinks").put(r)),
    ...seed.itemAssets.map((r) => tx.objectStore("itemAssets").put(r)),
    ...seed.trackers.map((r) => tx.objectStore("trackers").put(r)),
    ...seed.trackerSheets.map((r) => tx.objectStore("trackerSheets").put(r)),
