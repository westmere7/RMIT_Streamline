...
export async function linksTouching(db: StreamlineDatabase, itemIds: string[]): Promise<ItemLink[]> {
  ...
  for (const id of itemIds) {
    ...
    // Links written before field exclusions or hand pairings existed have
    // neither; read them as "sync everything the names match".
    for (const link of [...asA, ...asB]) seen.set(link.id, { ...link, excluded: link.excluded ?? [], pairs: link.pairs ?? [] });
  }
...
export class LocalItemLinkRepository implements ItemLinkRepository {
  ...
  async listByWorkspace(workspaceId: string): Promise<ItemLink[]> {
    ...
    return links.map((link) => ({ ...link, excluded: link.excluded ?? [], pairs: link.pairs ?? [] })).sort((a, b) => a.createdAt.localeCompare(b.createdAt));
  }

  async getById(id: string): Promise<ItemLink | null> {
    const db = await this.conn.getDb();
    const link = await db.get("itemLinks", id);
    return link ? { ...link, excluded: link.excluded ?? [], pairs: link.pairs ?? [] } : null;
  }
  ...
  async create(input: ItemLinkInput): Promise<ItemLink> {
    ...
    if (existing) return { ...existing, excluded: existing.excluded ?? [], pairs: existing.pairs ?? [] };
    const link: ItemLink = {
      id: newId(),
  ...
  async update(id: string, patch: Partial<Pick<ItemLink, "excluded" | "pairs">>): Promise<ItemLink> {
    ...
    const updated: ItemLink = { ...existing, excluded: existing.excluded ?? [], pairs: existing.pairs ?? [], ...patch, id };
