...
export const WORKSPACE_COLUMNS = "id, name, slug, logo_url, booking_key, booking_form, creative_team_name, asset_rates, created_at, updated_at";

export function toWorkspace(row: WorkspaceRow): Workspace {
  return { id: row.id, name: row.name, slug: row.slug, logoUrl: row.logo_url, bookingKey: row.booking_key ?? null, bookingForm: row.booking_form ?? null, creativeTeamName: row.creative_team_name ?? null, assetRates: normaliseAssetRates(row.asset_rates), createdAt: row.created_at, updatedAt: row.updated_at };
}
