    ...
    <div className={cn("flex h-12 items-end gap-[3px]", className)} role="img" aria-label={`${label}: ${values.map(formatCount).join(", ")}`}>
      {values.map((v, i) => (
        <div key={i} className="group relative flex h-full flex-1 flex-col justify-end" title={`${MONTH_LABELS[i]}: ${formatCount(v)}`}>
          <div
            className={cn("w-full rounded-sm transition-[height] duration-700 ease-kinetic motion-reduce:transition-none", i === nowMonth ? "ring-1 ring-offset-1 ring-offset-card ring-current" : "")}
            style={{ height: `${revealed ? Math.max(v > 0 ? 6 : 2, (v / max) * 100) : 0}%`, background: color, opacity: v > 0 ? (i === nowMonth ? 1 : 0.75) : 0.18 }}
          />
        </div>
      ))}
    </div>
