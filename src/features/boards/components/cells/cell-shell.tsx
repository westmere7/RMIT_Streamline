...
export function CellShell({ width, interactive = true, align = "left", className, children, ...props }: CellShellProps) {
  ...
  return (
    <div
      ...
      style={style}
      className={cn(
        "flex h-full shrink-0 items-center overflow-hidden border-r border-border/50 px-1 text-[13px] max-md:px-1.5 max-md:text-[14px]",
...
export function PopoverCell({ width, trigger, children, disabled, align = "left", contentClassName, ariaLabel, testId }: PopoverCellProps) {
  ...
  const triggerClassName = cn(
    "flex h-full shrink-0 items-center overflow-hidden border-r border-border/50 px-1 text-left text-[13px] max-md:px-1.5 max-md:text-[14px] transition-colors hover:bg-black/[0.02] focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-ring dark:hover:bg-white/[0.03]",
    alignment === "center" && "justify-center",
